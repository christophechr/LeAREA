...
const PopupCustom = ({mode}) => {
    ...
    return(
        <Popup modal nested onOpen={() => setopen(o => !o)} trigger={<button className="cross-button">+</button>} position="right center"
       contentStyle={{background: 'rgba(255,255,255,1)', height : 400, borderRadius : 10}}>
        <div>{
            data.map((val) => {
                return(
                    <button className="block">{val.name}</button>
...
const NewWorkflow = () => {
    ...
    return(
        ...
        <div className="newWorkflow">
            <input style={{width : 150}} placeholder="new workflow name"></input>
